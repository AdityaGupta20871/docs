...
```js
const clusters: Cluster[] = await spheron.organizations.getClusters({
  skip: 0,
  limit: 10
});
```
...
interface InstancesInfo {
    provisioned: number; //number of active instances in cluster
    provisioning: number; //number of starting instances in cluster
    failedToProvision: number; //number of fail to start instances in cluster
...
- `configuration`: the name of the variable.
  ...
  - `scale`: (number) - number of instances that should be created.
  ...
  - `machineImageId`: (string) - id of machine image which should be used for deploying instance. List of available can be aquierd from [get section under compute machine section](/sdk/compute/#get-4)
  - `healthCheckConfig`:
...
- `EnvironmentVariable`: 
  ...
  - `isSecret`: (string) - should environment variable be secret
...
await spheron.instance.update(
  instanceId,
  {
    environmentVariables: [{ key: "key", value: "value", isSecret: false }],
    commands: ["executable"],
    args: ["param"],
    tag: "latest";
  }
...
- `healthCheck`:
  ...
  - `cointainerPort`: (number) - container port on which health check should be done
...
interface InstanceDeployment {
  ...
  connectionUrls: string; //list of connection url for the instance
...
```js
const instanceLogs: Array<string> = await spheron.instance.getInstanceDeploymentLogs(
    deploymentId,
    {
      from: 0;
      to: 1000;
...
```js
  enum InstanceLogType {
    ...
    INSTANCE_LOGS, //the logs of container running on instance
    INSTANCE_EVENTS //the logs of instance events
}
```

Returns an list of for instance deployment.
---
...
- `createConfig`:
  ...
  - `environmentVariables`: (EnvironmentVariable[]) - list of variables for specified marketplace app. List of available environemt variables can be found in [MarketplaceApp object](/sdk/compute/#get-3)
...
- `options`:
  ...
  - `link`: (string) - one of the `connectionUrls` list elements of a active deployment of the instance to which domain will point to
