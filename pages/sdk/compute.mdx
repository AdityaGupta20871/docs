...
interface Cluster {
  ...
  createdBy: string; //user that created cluster
...
interface ClusterFundsUsage {
    dailyUsage: number; //cluster daily usage in uakt
    usedTillNow: number; //cluster daily usage in uakt
}
```
...
interface InstanceDetailed {
  ...
  agreedMachineImageType: MachineImageType; //compute machine information
...
interface MachineImageType {
    machineType: string; //compute machine type
...
interface MarketplaceInstanceResponse {
  ...
  instanceDeploymentId: string; //deploymentId
}
```
...
interface MarketplaceApp {
  ...
  serviceData: {
    dockerImage: string; //docker image used for marketplace app
    dockerImageTag: string; //docker image tag
    provider: string; //provider of docker image (currently only dockerhub is supported)
    variables: MarketplaceAppVariable[]; //list of enironemt variables
    ports: MarketplaceAppPort[]; //list of port mappings
